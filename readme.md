...
| 写法 | 单张耗时 | overlay 体积 |
| --- | --- | --- |
| PNG 默认压缩（2026-09-17 之前） | ~5.5 s | 19 MB |
| PNG `compress_level=1`（现默认） | 3.40 s | 32 MB |
| `--overlay-jpg`（JPEG q90/4:4:4） | **2.64 s** | 2.3 MB |
...
本地实测 7 张：`--jobs 1` 25.1s → `--jobs 4` **11.5s（2.2×）**，
且 CSV 与输出文件同 `--jobs 1` **逐位一致**（验证过）。默认 1（与旧行为一致）。
内存换吞吐：每张图峰值约 300MB（全分辨率图 + float 合成缓冲），jobs 别开太大。

还能做但**尚未做**：骨架化在全分辨率上要 918ms，可裁到掩码外接框内跑（结果不变）。
