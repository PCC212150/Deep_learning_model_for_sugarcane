...
**锚定是与标注口径配对的开关，不是独立选项**（`config.STEM_ANCHOR`，
inference.py / test.py 的 `--no-anchor` 可覆盖）：

| 标注怎么画 | 锚定 | 为什么 |
| --- | --- | --- |
| 从茎边开始画（当前 36 组） | **开**（缺省） | 真值含被泡沫环挡住的那段，预测不含 → 不补就系统性少算 |
| 只标看得见的根 | **关**（`--no-anchor`） | 两边都从可见处起算，再补等于单方面给预测加长度 |

错配的代价（2026-09-17 实测，7 张测试图，四个模型全部如此）：**总长 MAE 直接翻倍** ——
